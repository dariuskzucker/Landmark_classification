...
def early_stopping(stats, curr_count_to_patience, global_min_loss):
    ...
    """
    # DPME implement early stopping
    new_val_loss = stats[-1][1]
    
    if new_val_loss >= global_min_loss:
        curr_count_to_patience += 1
    else:
        global_min_loss = new_val_loss
...
def train_epoch(data_loader, model, criterion, optimizer, scheduler):
    """Train the `model` for one epoch of data from `data_loader`.

    Use `optimizer` to optimize the specified `criterion`
    """
    for i, (X, y) in enumerate(data_loader):
        # DONE implement training steps
